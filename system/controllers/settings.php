...
switch ($action) {
    case 'language-switch':
        ...
        $allowedLanguages = ['english', 'thai'];
        if (!in_array($selected_language, $allowedLanguages)) {
            $selected_language = 'english';
        }
        $_SESSION['user_language'] = $selected_language;
        setcookie('user_language', $selected_language, time() + 86400 * 365, "/");
        r2($_SERVER['HTTP_REFERER'] ?? getUrl('dashboard'), 's', Lang::T(ucwords($selected_language)));
        break;
